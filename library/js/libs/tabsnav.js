...
;(function(window) {
	...
	TabsNav.prototype._openTab = function(tab) {
		...
		this.DOM.bars.forEach(function(bar) {
			...
			// bar.style.transformOrigin = self.options.layout === 'vertical' ? '0% 0%' : '0% 0%';
		});
		...
		animeBars[this.options.layout === 'vertical' ? 'width' : 'height'] = animeBarsScale;
	...
	TabsNav.prototype._closeTab = function(tab) {
		...
		animeBars[this.options.layout === 'vertical' ? 'width' : 'height'] = 1;
		animeTabs.complete = function() {
			// Reset z-indexes.
			tab.style.zIndex = 1;
			self.isAnimating = false; 
	...
	TabsNav.prototype.show = function(callback) {
		...
		animeBars[this.options.layout === 'vertical' ? 'width' : 'height'] = [0,1];
		anime(animeBars);
	...
	TabsNav.prototype.hide = function(callback) {
		...
		animeBars[this.options.layout === 'vertical' ? 'width' : 'height'] = [1,0];
		anime(animeBars);
	...
	TabsNav.prototype._resize = function() {
		...
		if( this.isOpen ) {
			...
			var currentTab = this.DOM.tabs[this.current],
				...
				scaleStr = this.options.layout === 'vertical' ? 'width' : 'height';
			...
			this.DOM.tabs.forEach(function(tab, idx) {
				if( self.options.movable === 'all' || self.options.movable === 'single' && idx === self.current ) {
					...
					bar.style.height = sVal + 'px';
					if (self.current === 0) {
						document.querySelector('.homepage-proper').style.paddingTop = sVal + 'px';
					}
				}
			});
